...
# time settings
t_start = [3*hour,6*hour,9*hour] 
t_end = [4*hour, 7*hour,10*hour]
...
for i,u in enumerate(prop):
    filename += '_%s'% u
for i,u in enumerate(t_start):
    filename += '_%s_%s' % (int((u)/3600.), int((t_end[i])/3600.))
    ...
    for j,v in enumerate(prop):
        for k,w in enumerate(t_start):
            if readprof:
                ...
                time = data['time']
                t_start_in = find_nearest(time,w)
                ...
                if timeseries:
                    ...
                    zmin_in = find_nearest(z,zmin)
                    zmax_in = find_nearest(z,zmax)
                    plt.plot(p[zmin_in:zmax_in],z[zmin_in:zmax_in],marker[k], linewidth=0.8)
            elif readtmser:
