...
row1 = np.array([1,5,9,13])
row2 = np.array([1,5,9,13])+1
row3 = np.array([1,5,9,13])+2
row4 = np.array([1,5,9,13])+3
rows = np.zeros((4,4))
rows[:,0] = row1
rows[:,1] = row2
rows[:,2] = row3
        ...
        for r in range(1):
            ...
            for i,v in enumerate(turid):
                if v in rows[:,r]: 
                    ...
                    P[k,0] = v
                    if k == 0:
                        while turdata[kk,1] <= 3600*t:
                            kk += 1
                    P[k,1] = np.mean(turdata[kk:kk+900,3])

                    k += 1
            #P = np.flipud(np.sort(P,axis=0))
            #Pmean = Pmean + P
            plt.plot(x,P/P[0,1],label = t)
