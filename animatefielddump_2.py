...
pfull = pfull[t_start_in:t_end_in,plane/dz,:,:]
...
def update(frame):
    ...
    z = pfull[frame,:,:]
    levmin = np.amin(pfull[max(frame-16,0):min(frame+16,t_end_in),:,:])
    levmax = np.amax(pfull[max(frame-16,0):min(frame+16,t_end_in),:,:])
...
os.system('mv animtemp.mp4 %s' % figurepath )
